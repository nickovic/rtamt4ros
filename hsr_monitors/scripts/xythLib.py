...
class DWAcontroller:
    ...
    def plotResult(self, ax, targetV, targetW, vitualObstacles, dim):
        ...
        xmax = self.maxV*self.estimateTime*0.5
        ymax = self.maxV*self.estimateTime*0.8
        ...
        handles.append(pathFollow_handle)
...
# generating gauss wave
def gauss(x, a = 1, mu = 0, sigma = 1):
    return a * np.exp(-(x - mu)**2 / (2*sigma**2))

# generating square wave noise
def square(cycle, duty, topAmp, bottomAmp, endTime):
    time = []
    time.append(0)
    amp = []
    amp.append(bottomAmp)
    while time[-1] < endTime+1:
        time.append(time[-1]+cycle*(1-duty))
        amp.append(topAmp)
        time.append(time[-1]+cycle*(duty))
        amp.append(bottomAmp)
    #print(time)
    #print(amp)
    square_noise = interpolate.interp1d(time, amp, kind='previous')

    return square_noise
    
# generating square wave noise
def square1shot(start, depth, topAmp, bottomAmp, endTime):
    time = []
    time.append(0)
    amp = []
    amp.append(bottomAmp)
    ...
    time.append(start)
    amp.append(topAmp)
    time.append(start+depth)
    amp.append(bottomAmp)
    time.append(endTime+1.0)
    amp.append(bottomAmp)
    #print(time)
    #print(amp)
    square_1shot_noise = interpolate.interp1d(time, amp, kind='previous')

    return square_1shot_noise

def mdistEgo(data, scenarioParams, worldInfo):
    obstacle1translation = worldInfo["obstacle1translation"]
    obstacle1size = worldInfo["obstacle1size"]
    polyOb1 = posSizeRect2poly((obstacle1translation[0], obstacle1translation[2]), (obstacle1size[0], obstacle1size[2]))
    disttegoob1 = distTraj2Po(polyOb1, data["tegoodox"], data["tegoodoy"])
    obstacle2translation = worldInfo["obstacle2translation"]
    obstacle2size = worldInfo["obstacle2size"]
    polyOb2 = posSizeRect2poly((obstacle2translation[0], obstacle2translation[2]), (obstacle2size[0], obstacle2size[2]))
    disttegoob2 = distTraj2Po(polyOb2, data["tegoodox"], data["tegoodoy"])
    disttegotag1 = distTraj2Traj(data["tegoodox"], data["tegoodoy"], data["tagent1odox"], data["tagent1odoy"])
    disttegotag2 = distTraj2Traj(data["tegoodox"], data["tegoodoy"], data["tagent2odox"], data["tagent2odoy"])
    disttegopa1 = distTraj2Po(scenarioParams.prohibitArea1, data["tegoodox"], data["tegoodoy"])
    disttegopa2 = distTraj2Po(scenarioParams.prohibitArea2, data["tegoodox"], data["tegoodoy"])
    mdistego = []
    for Tdeob1, Tdeob2, Tepa1, Tepa2, Teag1, Teag2, in zip(disttegoob1, disttegoob2, disttegopa1, disttegopa2, disttegotag1, disttegotag2):
        mdistego.append( (Tdeob1[0], np.min([Tdeob1[1], Tdeob2[1], Tepa1[1], Tepa2[1], Teag1[1], Teag2[1]])) )
